...
EventHandlers.add(:on_calling_wild_battle, :roaming_pokemon,
  proc { |pkmn, level, handled|
    ...
    handled[0] = pbRoamingPokemonBattle(pkmn, level)
  }
)

def pbRoamingPokemonBattle(pkmn, level)
  ...
  EventHandlers.trigger(:on_wild_battle_end, pkmn.species, level, decision)
