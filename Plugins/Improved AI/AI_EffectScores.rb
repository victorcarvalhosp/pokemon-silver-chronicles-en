  ...
  def pbGetMoveScoreFunctionCode(score, move, user, target, skill = 100)
    ...
    when "StartHailWeather"
      if @battle.pbCheckGlobalAbility(:AIRLOCK) ||
        ...
        if PluginManager.installed?("Generation 9 Pack") && Settings::HAIL_WEATHER_TYPE != 1 && user.pbHasType?(:ICE) && skill >= PBTrainerAI.highSkill
          score += 40
        elsif user.pbHasType?(:ICE)
          score += 15
        elsif !(PluginManager.installed?("Generation 9 Pack") && Settings::HAIL_WEATHER_TYPE == 0) && user.ability != :SLUSHRUSH && user.ability != :ICEBODY && user.ability != :SNOWCLOAK
          score -= 90
        end
